...
static void PlotEx(ImGuiPlotType plot_type, const char *label, int series_count, void (*series_start)(int series),
                   float (*values_getter)(int series, void *data, int idx), void (*series_end)(int series), void *data,
                   ...
                   ImVec2 graph_size)
{
  ...
  const bool hovered = ImGui::ItemHoverable(inner_bb, id, 0);
  ...
  if (values_count > 0) {
    ...
    for (int s = 0; s < series_count; s++) {
      ...
      const ImU32 col_base =
        ImGui::GetColorU32((plot_type == ImGuiPlotType_Lines) ? ImGuiCol_PlotLines : ImGuiCol_PlotHistogram);
      const ImU32 col_hovered = ImGui::GetColorU32((plot_type == ImGuiPlotType_Lines) ? ImGuiCol_PlotLinesHovered
  ...
  // Text overlay
  if (overlay_text) {
    ImGui::RenderTextClipped(ImVec2(frame_bb.Min.x, frame_bb.Min.y + style.FramePadding.y), frame_bb.Max, overlay_text,
...
void PlotLinesSeries(const char *label, int series_count, void (*series_start)(int series),
                     float (*values_getter)(int series, void *data, int idx), void (*series_end)(int series),
                     void *data, int values_count, int values_offset, const char *overlay_text, float scale_min,
                     float scale_max, ImVec2 graph_size)
{
  PlotEx(ImGuiPlotType_Lines, label, series_count, series_start, values_getter, series_end, data, values_count,
         values_offset, overlay_text, scale_min, scale_max, graph_size);
}

} // namespace ImGuiExt

ArrowWidget::ArrowWidget(float3 direction) { quatFromDirection(mDirectionQuat, direction); }
  ...
  if (ImGui::IsItemActive()) {
    ...
    if (ImGui::IsMouseClicked(0)) {
      ...
      if (n0 > FLT_EPSILON && n1 > FLT_EPSILON) {
        ...
        float nqorig =
          sqrt(origQuat.x * origQuat.x + origQuat.y * origQuat.y + origQuat.z * origQuat.z + origQuat.w * origQuat.w);
...
void ArrowWidget::drawTriangles(ImDrawList *draw_list, const ImVec2 &offset, const ImVector<ImVec2> &triProj,
                                const ImVector<ImU32> &colLight, int numVertices)
{
