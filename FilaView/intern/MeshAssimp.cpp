...
std::string shader_from_config(const MaterialConfig &config)
{
  ...
  if (config.cst_shade) {
    shader += R"SHADER(
      material.ambientOcclusion = 0.3;
    )SHADER";

    if (config.tex_shiness) {
      shader += "float2 uv_specular = getUV" + std::to_string(config.uv_shiness) + "();\n";
      shader += R"SHADER(
        material.specularFactor = texture(materialParams_specularColorMap, uv_specular).r; 
      )SHADER";
    }
  }
  ...
  if(config.cst_shade) {
    shader += R"SHADER(
        vec3 surfaceShading(const MaterialInputs material, const ShadingData shading, const LightData light){
          vec3 baseClr = material.baseColor.rgb;
          vec3 clr = light.NdotL + vec3(material.ambientOcclusion); 
          clr = clr * baseClr * light.colorIntensity.rgb;
    )SHADER";

    if(config.tex_shiness) {
      shader += R"SHADER(
          vec3 hfdir = normalize (shading_view + light.l);
          float spec = pow(max(dot(shading_normal, hfdir), 0.0), 32.0);
          clr += material.specularFactor * vec3(spec) * light.colorIntensity.rgb;
      )SHADER";
    }
...
Material *create_material_from_config(Engine &engine, MaterialConfig &config)
{
  ...
  builder.customSurfaceShading(config.cst_shade);
...
void MeshAssimp::build_materials(filament::Engine *engine) 
{
  ...
  auto fun2 = [this, engine](MaterialInstance *mtl, uint64_t tex_id) {
    ...
    mtl->setParameter(tex_name[tc->type].c_str(), _textures[tex_id], sampler);
  };
...
void MeshAssimp::adjust_material_config(MaterialConfig *material) 
{
  ...
  if (material->tex_diffuse || material->tex_specular || material->tex_shiness)
    material->cst_shade = true;
}
