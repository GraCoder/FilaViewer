...
TWin *TWin::create(TWin *) { return new FTWin(nullptr); }
...
void FTWin::realize() 
{
  ...
  uint32_t windowFlags = SDL_WINDOW_SHOWN | SDL_WINDOW_ALLOW_HIGHDPI | SDL_WINDOW_RESIZABLE | SDL_WINDOW_BORDERLESS;

  auto swin = SDL_CreateWindow("", x, y, _width, _height, windowFlags);
  _window = swin;
  //_swapchain = _engine->createSwapChain(nativeWindow(swin), filament::SwapChain::CONFIG_HAS_STENCIL_BUFFER);
